...
class ScheduleOnlineTestAPIView(generics.ListAPIView):
    serializer_class = ApplicantSerializer1

    def get_queryset(self):
        return ApplicationDetails.objects.filter(application_status__in=[2, 3])
...
class ScheduledUserDetailsAPIView(generics.ListAPIView):
    serializer_class = ScheduledUserDetailsSerializer
    ...
    def get_queryset(self):
        ...
        return {'applicants1': applicants1, 'applicants2': applicants2}

    def list(self, request, *args, **kwargs):
        queryset = self.get_queryset()
        serializer1 = self.get_serializer(queryset['applicants1'], many=True)
        serializer2 = self.get_serializer(queryset['applicants2'], many=True)
        is_hr = request.user.is_authenticated and User_Rolls.objects.filter(user_id=request.user.id, roll_id=1).exists()
        is_hrhead = request.user.is_authenticated and User_Rolls.objects.filter(user_id=request.user.id, roll_id=3).exists()
